...
export const CaddBotUser = async (req: Request, res: Response) => {
  try {
    let data = await addBotUser(req.body.name, Math.floor(Math.random() * 100) + 1, "");
    res.json({ status: 201, message: "Successfully created Bot User" });
  } catch (error) {
...
export const CupdateBotUser = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { imageName: img } = req as any;

    let data = await updateBotUser(id as any, req.body.name, img);
    res.json({ status: true, data });
  } catch (error) {
...
export const CdeleteBotUser = async (req: Request, res: Response) => {
  try {
    let data = await deleteBotUser(req.params.id as any);
    res.json({ status: true, data });
  } catch (error) {
