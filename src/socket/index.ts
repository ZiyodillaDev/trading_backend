...
let socketUsers = {} as {
  [key: string]: { userId: string; fToken: string; sToken: string };
};
    ...
    switch (GameState) {
      case "BET":
        if (Date.now() - startTime > BETINGTIME) {
          ...
          NextState = "PLAYING";
          startTime = Date.now();
          gameTime = await getRandom();
          // console.log(gameTime,"=======>gameTime");
          const time = Date.now() - startTime;
          mysocketIo.emit("gameState", {
            currentNum,
            currentSecondNum,
            GameState,
            time,
      ...
      case "PLAYING":
        ...
        if (RTPAmount >= RTP) target = currentNum;
      ...
      case "GAMEEND":
        if (Date.now() - startTime > GAMEENDTIME) {
          ...
          let interval = setInterval(() => {
            bet(botIds[i], i);
            i++;
            if (i > botUsers.length - 1) clearInterval(interval);
          }, 100);
          startTime = Date.now();
          GameState = "BET";
          NextState = "READY";
...
const getRandom = async () => {
  var r = Math.random();
  const data = await getGameNumber();
  console.log(data);
  if (data.length > 0) {
    const index = count % data.length;
    target = data ? data[index].amount : 1 / r;
  } else {
    target = 1 / r;
  }
  console.log(target);
...
setInterval(() => {
  if (GameState === "PLAYING") {
    let _bots = botIds.filter(
      (k) => users[k] && users[k].f.target <= currentNum && users[k].f.betted
    );
    if (_bots.length) {
      for (let k of _bots) {
    ...
    _bots = botIds.filter(
      (k) => users[k] && users[k].s.target <= currentNum && users[k].s.betted
    );
    if (_bots.length) {
      for (let k of _bots) {
...
async function getDBBots() {
  let users = await getBotUsers();
  return users;
}
// Bots bet in here.
async function bet(id: string, index: number) {
  let fbetAmount = Math.random() * 100 + 1;
  let sbetAmount = Math.random() * 100 + 1;
  let randomNumber = Math.ceil(Math.random() * 10);
  let botUsers = await getDBBots();
  let i = index % botUsers.length;
  users[id] = {
    ...DEFAULT_USER,
    userName: botUsers.length > 0 ? botUsers[i].name : randomstring.generate(4),
...
export const initSocket = (io: Server) => {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("disconnect", async () => {
      const checkIndex = sockets.findIndex((s) => s.id === socket.id);

      if (checkIndex !== -1) {
        console.log("Disconnected User : ", socket.id);
        if (
          userIds[socket.id] &&
          users[userIds[socket.id]] &&
          users[userIds[socket.id]].userType
        ) {
          let userData = await DUsers.findOne({ name: userIds[socket.id] });
          const refoundAmount = await axios.post(
            config.reFundURL,
            {
    ...
    socket.on("enterRoom", async (props) => {
      ...
      socket.emit("getBetLimits", {
        max: bettingLimits.maxBetAmount,
        min: bettingLimits.minBetAmount,
      });
      let balance = 5000;
      let id = socket.id;
      let userName = name ?? uuidv4();
      let userType = false;
      let img = "";
      if (token !== null && token !== undefined) {
        let tokenSplit = token.split("&");
        const getUserInfo = await axios.post(config.getUserTokenURL, {
          token: tokenSplit[0],
          ptxid: uuidv4(),
        });
    ...
    socket.on("playBet", async (data) => {
      ...
      if (GameState === "BET") {
        ...
        socket.emit("error", {
          message: "You can't bet. Try again at next round!",
          index: type,
        });
      }
    });
    socket.on("cashOut", async (data) => {
      ...
      let u = copyObject(users[userIds[socket.id]]);
      let player;
      if (type === "f") player = u.f;
      else if (type === "s") player = u.s;
      if (!!u) {
        if (GameState === "PLAYING") {
          if (!player.cashouted && player.betted) {
            if (endTarget <= currentSecondNum) {
              ...
              socket.emit("success", {
                message: "You have cashed out!",
                target: Number(player.target).toFixed(2),
                amount: Number(player.cashAmount).toFixed(2),
              });
            } else {
              socket.emit("error", {
                message: "You can't cash out!",
                index: type,
              });
            }
          }
      ...
      } else socket.emit("error", { message: "Undefined User", index: type });
    });
